...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String id, Drivable thing) {
    ...
    public boolean hasFasterThan(int speed) {
        Drivable[] things = this.drivable_map.values().toArray(new Drivable[0]);
        for (int i =0; i < things.length; i ++) {
            if (things[i].getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public List getTradable(){
        HashMap<String, Drivable> items= new HashMap<String, Drivable>();
        Drivable[] things = this.drivable_map.values().toArray(new Drivable[0]);
        for (int i =0; i < things.length; i ++) {
            if (things[i] instanceof Tradable){
                items.put(String.valueOf(i), things[i]);
            }
        }
        return List.of(items.values().toArray(new Drivable[0]));
    }
